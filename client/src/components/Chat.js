...
function Chat() {
  ...
  const [from, setNickname] = useState('');
  ...
  useEffect(() => {
    ...
    socket.on('chat message', ({ from, msg }) => {
      setChat([...chat, { from, msg }]);
    });

    socket.on('private msg', ({ from, msg }) => {
      setChat([...chat, `🔒 Private Message from ${from}: ${msg}`]);
    });
  ...
  const submitMsg = (e) => {
    ...
    if (msg === '') {
      ...
      socket.emit('chat message private', { from, to, msg });
      setChat([...chat, { from, msg }]);
      setChat([...chat, `🔒 Private Message for ${to}: ${msg}`]);
      setMsg('');
      setTo('');
    } else {
